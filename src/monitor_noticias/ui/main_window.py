...
from PySide6.QtWidgets import (
    QFrame, QHBoxLayout, QLabel, QMainWindow, QMenu, QPushButton, QStackedWidget,
...
class MainWindow(QMainWindow):
    """Janela principal equivalente ao DashboardV5 Desktop.

    A navegação ativa usa menu lateral + QStackedWidget. Fechar oculta para o
    tray, como a janela Compose. Encerramento real ocorre pela ação Sair.
    """
    ...
    def __init__(self, controller: MainUiController | None = None, paths: AppPaths | None = None) -> None:
        ...
        self.controller = controller or MainUiController.create_default(
            self.paths, news_sources=NEWS_SOURCES, video_sources=VIDEO_SOURCES,
        ...
        self._timer = QTimer(self); self._timer.setInterval(250); self._timer.timeout.connect(self._tick); self._timer.start()
        self.navigate(Section.HOME)

    def _build_ui(self) -> None:
        root = QWidget(); root.setObjectName("root"); self.setCentralWidget(root)
        outer = QHBoxLayout(root); outer.setContentsMargins(0,0,0,0); outer.setSpacing(0)
        self.sidebar = QFrame(); self.sidebar.setObjectName("sidebar"); self.sidebar.setFixedWidth(258)
        side = QVBoxLayout(self.sidebar); side.setContentsMargins(14,16,14,14); side.setSpacing(5)
        brand = QLabel("MONITOR DE NOTÍCIAS"); brand.setObjectName("brandTitle"); side.addWidget(brand)
        sub = QLabel("Centro de monitoramento • Windows"); sub.setObjectName("brandSub"); side.addWidget(sub); side.addSpacing(12)
        self.nav_buttons: dict[Section,QPushButton] = {}
        for section in SECTION_ORDER:
            button = QPushButton(f"{section.value.icon}   {section.value.label}"); button.setObjectName("navButton"); button.setCheckable(True); button.setMinimumHeight(36)
            button.clicked.connect(lambda _=False,s=section:self.navigate(s)); side.addWidget(button); self.nav_buttons[section]=button
        side.addStretch(); self.sidebar_status = QLabel(); self.sidebar_status.setObjectName("brandSub"); self.sidebar_status.setWordWrap(True); side.addWidget(self.sidebar_status)
        ver=QLabel("Windows Portable v4.0.2"); ver.setObjectName("brandSub"); side.addWidget(ver); outer.addWidget(self.sidebar)
    ...
    def _build_tray(self) -> None:
        ...
        exit_action=menu.addAction("Sair"); exit_action.triggered.connect(self.exit_application)
    ...
    def _tick(self) -> None:
        ...
        self.footer_left.setText("Busca em andamento" if state.news_busy or state.video_busy else "Sistema operacional")
        self.footer_right.setText(f"Notícias: {state.status}   •   Vídeos: {state.video_status}")

    def _state_changed(self, _state) -> None:
        # Callbacks podem vir de workers; o timer Qt atualiza widgets no thread GUI.
        pass
