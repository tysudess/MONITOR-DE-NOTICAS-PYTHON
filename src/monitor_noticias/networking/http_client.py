...
class HttpClient:
    ...
    def from_proxy_settings(cls, proxy_settings: "ProxySettings", *, session: requests.Session | None = None) -> "HttpClient":
        """Resolve o proxy a cada requisição para refletir alterações da UI sem reiniciar."""
    ...
    def get_text(self, url: str, *, headers: dict[str, str] | None = None, connect_timeout: float, read_timeout: float, max_body_bytes: int | None = None) -> str:
        response = self.session.get(url, headers=headers or {}, timeout=(connect_timeout, read_timeout), allow_redirects=True, proxies=self._proxies())
    ...
    def post_json(self, url: str, payload: dict[str, Any], *, headers: dict[str, str], connect_timeout: float, read_timeout: float) -> dict[str, Any]:
        response = self.session.post(url, json=payload, headers=headers, timeout=(connect_timeout, read_timeout), allow_redirects=True, proxies=self._proxies())
        response.raise_for_status()
