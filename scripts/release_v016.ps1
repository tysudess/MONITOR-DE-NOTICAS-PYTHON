...
python -m pytest -q
python scripts/capture_v016_layout_only.py

$run=Get-Content run.py -Raw
if($run -match 'install_v015_image_truth_layout\(\)'){throw 'Camada v0.0.15 que ocultava abas ainda ativa'}
if($run -match 'install_v015_home_reference\(\)'){throw 'Substituicao funcional da Home ainda ativa'}
if($run -notmatch 'install_v016_layout_only\(\)'){throw 'Camada v0.0.16 ausente'}
...
$extract=Join-Path $env:RUNNER_TEMP 'v013-source-v016'
...
$audit=Join-Path $env:RUNNER_TEMP 'ffmpeg-v016';New-Item -ItemType Directory -Force $audit | Out-Null
...
try { ./scripts/validate_portable.ps1 -ZipPath $newZip }
  ...
  if(-not $payload.ok -or [int]$payload.player_position_ms -lt 300){throw 'Smoke invalido'}
}
...
$notes=@"
Monitor de Noticias Python 0.0.16 — Windows Portable x64

- refazimento do layout usando as imagens fornecidas como verdade visual
- nenhuma pagina funcional substituida pelo layout
- nenhuma aba existente ocultada ou removida
- Extrator de Noticias restaurado na navegacao e preservado como integracao original
- Automacao Planilhas restaurada na navegacao e preservada como integracao original
- Home original mantida; apenas aparencia refinada
- sidebar, topbar, hero, cards, campos, tabelas e controles ajustados visualmente
- motores, banco, coletores, matching, automacao, proxy, credenciais, integracoes, FFmpeg/FFprobe e exportacoes preservados
- regressao completa, gate de preservacao funcional, capturas das telas e portable recem-gerado validados antes da publicacao

SHA-256 do portable: $hash
"@
gh release create $Tag "publish/$Portable" publish/SHA256SUMS.txt publish/FFMPEG_SOURCE_INFO.txt publish/INTEGRATIONS-MANIFEST.json --target $target --title 'Monitor de Noticias Python 0.0.16' --notes $notes
