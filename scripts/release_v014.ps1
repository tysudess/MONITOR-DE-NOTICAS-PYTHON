...
python -m pytest -q
python scripts/capture_v014_reference.py

$run=Get-Content run.py -Raw
if($run.IndexOf('install_v014_full_reference_layout()') -lt $run.IndexOf('install_v013_sidebar_reference()')){throw 'v0.0.14 fora de ordem'}
if($run.IndexOf('install_v014_reference_sidebar_local()') -lt $run.IndexOf('install_v014_full_reference_layout()')){throw 'sidebar v0.0.14 fora de ordem'}
...
if(-not $zip){throw 'Portable v0.0.13 ausente'}
if((Get-FileHash $zip.FullName -Algorithm SHA256).Hash.ToLowerInvariant() -ne $PrevHash){throw 'Hash v0.0.13 divergente'}
$extract=Join-Path $env:RUNNER_TEMP 'v013-source'
...
if(-not $root){throw 'Raiz v0.0.13 ausente'}
...
$audit=Join-Path $env:RUNNER_TEMP 'ffmpeg-v014'
...
$notes=@"
Monitor de Notícias Python 0.0.14 — Windows Portable x64

- novo shell visual global baseado nas referências aprovadas
- barra superior com busca real, proxy, automação, data/hora e temperatura
- sidebar compacta azul-marinho/ciano e cabeçalhos harmonizados
- cards, campos, botões, superfícies e rodapé refinados
- motores, banco, coletores, matching, automação, proxy, credenciais e persistência preservados
- Editor de PDF, Extrator de Vídeos, Editor de Vídeo, Capas e integrações externas preservados
- FFmpeg/FFprobe mantidos byte-a-byte a partir da versão validada anterior
- suíte completa, capturas de referência e portable recém-gerado validados

SHA-256 do portable: $hash
"@
gh release create $Tag "publish/$Portable" publish/SHA256SUMS.txt publish/FFMPEG_SOURCE_INFO.txt publish/INTEGRATIONS-MANIFEST.json --target $target --title 'Monitor de Notícias Python 0.0.14' --notes $notes
