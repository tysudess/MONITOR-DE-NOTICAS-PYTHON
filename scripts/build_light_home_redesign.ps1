...
python -c "from monitor_noticias.ui.v021_light_home_dashboard import LightHome, install_v021_light_home_dashboard; print('LIGHT_HOME_IMPORT_OK')"
if ($LASTEXITCODE -ne 0) { throw 'Validacao da Home clara falhou' }
...
$previousDir = Join-Path $env:RUNNER_TEMP 'light-home-previous'
...
if ($LASTEXITCODE -ne 0) { throw 'Download do portable base falhou' }
if ((Get-FileHash $previousZip -Algorithm SHA256).Hash.ToLowerInvariant() -ne $PrevHash) { throw 'Hash do portable base divergente' }

$extract = Join-Path $env:RUNNER_TEMP 'light-home-source'
...
$audit = Join-Path $env:RUNNER_TEMP 'light-home-ffmpeg'
...
$out = Join-Path $env:RUNNER_TEMP 'light-home-portable'
...
$artifactName = 'MONITOR-DE-NOTICIAS-PYTHON-light-home-redesign-portable-windows-x64.zip'
Copy-Item $zip (Join-Path $publish $artifactName) -Force
$hash = (Get-FileHash $zip -Algorithm SHA256).Hash.ToLowerInvariant()
"$hash  $artifactName" | Set-Content (Join-Path $publish 'SHA256SUMS.txt') -Encoding ascii
Write-Host 'LIGHT_HOME_PORTABLE_OK'
